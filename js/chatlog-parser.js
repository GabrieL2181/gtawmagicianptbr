$(document).ready(function() {
    ...
    function processOutput() {
        ...
        chatLines.forEach((line) => {
            ...
            if (line.includes("Use /anim stop para parar as animações e remover os itens de suas mãos.")) {
                return;
            }

            // Skip abuse warning messages
            if (line.includes("Qualquer abuso do editor resultará em punições severas.")) {
                return;
            }

            // Skip kick/ban messages
            if (line.includes("foi expulso por:") || line.includes("foi banido por:")) {
                return;
            }

            // Skip unfreeze instructions
            if (line.includes("⚠️Você pode agora pressionar K para se descongelar. Não segure W enquanto faz isso.")) {
                return;
            }

            // Skip animation stop info
            if (line.includes("[INFO] Se não funcionar, use .stop ou /anim stop.")) {
                return;
            }

            // Skip vehicle teleport info
            if (line.includes("[INFO] Quando o veículo for respawnado do mapa, você poderá usar /vlista  para recuperá-lo novamente.")) {
                return;
            }
            ...
            // Skip hat info
            if (line.includes("[INFO] Você também pode usar /hat custom1-5 para escolher seus chapéus favoritos salvos!")) {
                return;
            }

            // Skip animation error
            if (line.includes("[ERROR] Esta animação não foi encontrada.")) {
                return;
            }
            ...
            if (line.startsWith("[!]")) {
                ...
                if (hasToSection) {
                    // Extract the target name from (to CharacterName)
                    const toSectionMatch = lineWithoutExclamation.match(toSectionPattern);
                    const targetName = toSectionMatch ? toSectionMatch[0].match(/\(para ([^)]+)\)/i)[1].toLowerCase() : '';
    ...
    function applyUserCensorship(line) {
        ...
        try {
            ...
            console.warn('Falha na regex de censura, usando método alternativo:', error);
    ...
    function applySpecialFormatting(line, lowerLine) {
        ...
        if (lowerLine.includes("diz [lower]")) {
        ...
        if (line.includes("diz [baixo] (phone):") || line.includes("diz (celular):")) {
        ...
        if (lowerLine.includes("diz:") && !lowerLine.includes("[baixo]") && !lowerLine.includes("[lower]") && !lowerLine.includes("sussurros") && !lowerLine.includes("(phone)") && !lowerLine.includes("(viva-voz)")) {
        ...
        if (line.match(/^(?:\[\d{2}:\d{2}:\d{2}\]\s+)?\d+: .+/)) {
            ...
            if (line.includes("Dinheiro ($")) {
                const moneyMatch = line.match(/^(\d+: Dinheiro \()(\$\d+(?:,\d{3})*)(\) \(\d+g\))$/);
                if (moneyMatch) {
        ...
        if (lowerLine.startsWith("dinheiro em mãos:")) {
            return wrapSpan("green", line);
        }
        ...
        if (lowerLine.includes("diz (phone):") || lowerLine.includes("diz (viva-voz):")) {
        ...
        if (lowerLine.includes("você deu") ||
            lowerLine.includes("pagou você") ||
            lowerLine.includes("você pagou") ||
            lowerLine.includes("você recebeu"))
            return handleTransaction(line);

        if (lowerLine.includes("agora você está mascarado")) return wrapSpan("green", line);

        if (lowerLine.includes("você mostrou o seu inventário para")) return wrapSpan("green", line);
        ...
        // Convite para facção
        if (line.includes("Você recebeu um convite para se juntar à facção")) {
            const parts = line.split("juntar à facção ");
            const factionPart = parts[1].split(",")[0];
            return parts[0] + "juntar à facção " + wrapSpan("yellow", factionPart) + ", escreva /faccept para confirmar";
        }

        // Confirmação de entrada na facção
        if (line.includes("Agora você é membro de")) {
            const parts = line.split("membro de ");
            const factionPart = parts[1].split(" talvez")[0];
            return parts[0] + "membro de " + wrapSpan("yellow", factionPart) + " talvez seja necessário usar /switchfactions para defini-la como sua facção ativa!";
        }
        ...
        if (/Você acabou de pegar .+?! Você sentirá os efeitos da droga em breve\./.test(line)) {
        ...
        if (lowerLine.includes("diz:") && !lowerLine.includes("[baixo]") && !lowerLine.includes("[lower]") && !lowerLine.includes("sussurra") && !lowerLine.includes("(phone)") && !lowerLine.includes("(loudspeaker)")) {
            return formatSaysLine(line, currentCharacterName);
        }
...
function formatWeatherLine(line) {
    // Verifica se a linha é a de temperatura com condição do tempo
    if (line.startsWith("Temperatura:")) {
        const tempPattern = /^Temperatura:\s*([\d.]+°C)\s*\(([\d.]+°?F)\),\s*é\s*atualmente\s*([^.]+)\.?$/i;
        const tempMatch = line.match(tempPattern);
        if (tempMatch) {
            const [_, tempC, tempF, condition] = tempMatch;
            return wrapSpan("white", "Temperatura: ") +
                   wrapSpan("green", tempC) + " " +
                   wrapSpan("white", "(") +
                   wrapSpan("green", tempF) +
                   wrapSpan("white", ")") +
                   wrapSpan("white", ", é atualmente ") +
                   wrapSpan("green", condition) + ".";
        }
        return wrapSpan("white", "Temperatura: ") +
               wrapSpan("green", line.replace("Temperatura:", "").trim());
    }

    // Verifica se a linha é a de vento, umidade e chuva
    if (line.startsWith("Vento:")) {
        const windPattern = /^Vento:\s*([\d.]+)\s*km\/h\s*\(([\d.]+)\s*mph\),\s*umidade:\s*(\d+%)\s*,\s*precipitação de chuva:\s*([\d.]+)\s*mm\.?$/i;
        const windMatch = line.match(windPattern);
        if (windMatch) {
            const [_, windKmh, windMph, humidity, rain] = windMatch;
            return wrapSpan("white", "Vento: ") +
                   wrapSpan("green", windKmh + " km/h") + " " +
                   wrapSpan("white", "(") +
                   wrapSpan("green", windMph + " mph") +
                   wrapSpan("white", "), Umidade: ") +
                   wrapSpan("green", humidity) +
                   wrapSpan("white", ", Precipitação de chuva: ") +
                   wrapSpan("green", rain + " mm") + ".";
        }
        return wrapSpan("white", "Vento: ") +
               wrapSpan("green", line.replace("Vento:", "").trim());
    }

    // Ignora ou processa outras linhas, como "Previsão do tempo:"
    if (line.startsWith("Previsão do tempo")) {
        return wrapSpan("white", line.trim());
    }
...
function isRadioLine(line) {
    ...
    function handleWhispers(line) {
        ...
        const groupWhisperPattern = /^[A-Z][a-z]+\s[A-Z][a-z]+\ssussurou para \d+\spessoas/i;
        const match = line.match(groupWhisperPattern);
        if (match) {
            const splitIndex = match.index + match[0].length;
            return `<span class="orange">${line.slice(0, splitIndex)}</span><span class="sussurra">${line.slice(splitIndex)}</span>`;
        }

        return wrapSpan("sussurra", line);
    }    
